...
impl<'a> Lexer<'a> {
    ...
    pub fn lex(&mut self) -> Vec<LexerToken> {
        ...
        while self.position < self.length {
            ...
            self.position += 1;
            self.current_position = SourcePosition::new(None, self.current_position.line, self.current_position.column + 1);
        }
    ...
    pub fn find_new_token(&mut self, c: char, tokens: &mut Vec<LexerToken<'a>>) {
        ...
        match c {
            ...
            '\n' => {
                self.current_position = SourcePosition::new(None, self.current_position.line + 1, 1);
            }
